...
- **Three payloads carry paths, in three shapes, and the differences are deliberate.** `/api/car-performance` puts `preview` on the row, because a row there *is* a car; `team_previews` picks the first of the team's liveries that has one. `/api/driver-performance` carries a `previews` map keyed by **livery name**, because a row there is one entry bound to one livery — no "first car of the team" guess, and a per-track stand-in shows the car it actually drives that weekend. `/api/championships/:id/offers` carries a map keyed by **team name** beside the offers, because `contracts::Offer` is the terms of a deal and a picture is not one of them — and because the signed contract's own row wants the same picture while not being an offer at all. `champ_previews` is `team_previews` for one championship's roster.
- The driver-performance roster is **deduped** before `previews_for`: which model declares a shared name is settled by how much of the roster it declares, and a livery repeated by a dozen per-track entries is still one livery's worth of evidence.
- `liveryImg(path, class)` in `utils.js` is the only place the URL is built. All three surfaces call it, so the encoding, the lazy loading and the "no path, no image" rule cannot drift apart.

### Rows AMS2 will never field are hidden by default (`SHOW_PHANTOM_ENTRIES`, `utils.js`)

An entry naming a livery the game owns no car for is silently ignored, so it exists in the roster file and nowhere else. Both performance tabs mark those rows `no livery`, and both now **hide them unless asked** — they are noise in a table about how the grid performs, and the person who wants them is the one repairing the roster. On F-Junior they are 404 of 424 rows.

- **One preference asked twice, not two.** The checkbox appears in each tab's filter bar; `initPhantomToggles` keeps every copy in step, because both tabs are showing the same entries from the same files. A tab rebuilt after an edit comes back in whatever state was already chosen.
- Hiding is a class on `<body>` driving **one** CSS rule against `.carperf-class tr.driverperf-phantom` — `.carperf-class` is the section wrapper on both tabs, so neither can drift into showing what the other hides. It also drives the `(hidden)` suffix, so there is no JS rewriting label text.
- **Every class that hides rows says so in its own heading.** Driver Performance already counted them (`N entries AMS2 ignores`); Car Performance gained the same badge, reusing `.driverperf-phantom-count` so the suffix rule covers both. Without it a class quietly shows fewer cars than its roster names, and Driver Performance's own `(39 entries, 26 cars)` would read as a miscount.
- The count lives on the class heading and **not** on the checkbox, which only asks whether *any* class has one: a tab-wide total is dominated by whichever classes the class filter has hidden. `phantomToggleHtml` renders nothing when there is none, rather than offering a control that would do nothing.
- The toggle is untouched by **All**/**None**: those pick classes, this picks what counts as a row. Only `phantom === true` is hidden — `null` is *not checkable* (see `phantom_liveries`), and hiding those would empty the table for every class with no livery mod.
...
- **Only a per-track entry may be deleted** (`remove_driver_entry_str`, and `remove_track_entries_str` for a whole class). A regular entry is a car on the grid: removing one shrinks the field every expected finishing position is derived from and leaves a livery with nobody in it, so the writer refuses rather than trusting the button not to offer it. Both go through `write_with_backup`, so Reset to baseline undoes them like any other edit, and both routes answer with the **whole class** — every index below a removed row has shifted, and the table addresses rows by index. The bulk sweep cuts back to front so earlier offsets stay valid, and writes nothing when there was nothing to remove.
- `driver_name_at` resolves the name a span is *listed under* — its own, or the one it inherits — and both the edit guard and the delete guard call it, so a row whose name is not its own cannot end up editable but undeletable, or the reverse.
...
- **A new config and a config that predates a field are two different defaults.** A `#[serde(default = "…")]` in `config.rs` answers *"what did a config written before this field existed mean?"* and therefore may never move — `test_rating_tuning_defaults_for_existing_configs` pins it, and a career created under it stamped its own `rating_params` from it. `Config::default()` answers *"what should someone who has never configured anything start on?"*, and is free to be retuned. They currently differ on `poll_ms`, the contract salaries, prizes and buy-in rate, `rating_strictness`, `retirement_distance_pct`, `offer_margin` and `class_years` — but *not* on `starting_balance`, which is deliberately the same 100,000 in both. Machine-specific settings stay unset in both: `saves_dir`, `active_career`, `custom_ai_dir` and `spotter_voice` — a named voice that is not installed silently mutes the spotter, whereas unset means the system default.
- The shipped economy is **100,000 top salary / 50,000 floor, 3,000 a rating point, 1,500,000 champion prize, 100,000 founding balance**, with `rating_strictness: 5` and `offer_margin: 0` — so every bar is five points higher than results alone would ask and there is no middle tier at all: a bar missed is a seat refused outright.
- **Every shipped grid keeps a bought seat within reach; most offer a choice of them.** Without the first, a roster is one a new career cannot start on at all and `open_a_way_in`'s last-resort discount becomes the normal way in rather than the exception. At the shipped 100,000, six of the eight rosters put a second seat in reach too; F-Retro_Gen3 (90,000 then 113,999) and F-Classic_Gen2 (100,000 then 102,000) put only their cheapest. A balance near 115,000 would restore eight of eight — it is a tuning choice, and `test_a_new_career_can_buy_a_seat_on_every_shipped_grid_and_choose_on_most` re-measures both halves against the real rosters and fails loudly if retuning moves the costs out from under them.
- `default_starting_balance` (the serde fallback, for a config that predates the field) is held level with the shipped figure, so both found a career on the same money. They do **not** spend it in the same economy: the fallbacks around it are still the old ones at 150,000 a point, so a career founded on a field-less config affords nothing outright and gets in through `open_a_way_in`'s discount. The 5,000,000 it replaced was measured against that old economy, where the dearest second-cheapest seat cost 4,949,999.
- There being something to buy on every roster is `pay_driver_margin`'s doing. F-Vintage_Gen2 (one pay seat) and F-Classic_Gen3 (none) used to be named exceptions precisely because their back rows were reachable on merit and so were free rather than for sale; a team at the back now sells to anyone it does not actively want, and the exception list is gone.
- `contract_buy_in_per_point` is the knob that decides whether a shortfall costs pocket money or millions, so it has to be retuned together with the balance rather than on its own: at the old 150,000 a point a founding balance of 100,000 would not reach a single seat on any grid.
...
- **An unset `custom_ai_dir` is flagged, not defaulted.** There is no path worth guessing — an AMS2 install can be on any drive — but an empty one means this install has no rosters at all, which silently takes the rating, every team requirement, every offer, the money and the live grid's team names with it. So `flagMissingAiDir` in `config.js` unhides two things: the amber notice beside the box that fixes it (`#cfg-custom-ai-dir-missing`) and a `!` on the **Config tab button** itself (`#tab-config-warn`). The badge is the point — someone who has never opened the tab is exactly the person who has not set the folder — which is why `config.js` calls `loadConfig()` once at startup rather than only on tab click, and why `saveConfig` re-reads the flag from the response instead of waiting for a reload. `saves_dir` gets no such warning: unset there has a real answer (`championships` beside the executable), so there is nothing to fill in.
